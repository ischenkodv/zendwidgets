...
class App_View_Helper_Table extends Zend_View_Helper_Placeholder_Container_Standalone
{
     ...
     */
	protected $_regKey = 'App_View_Helpers_Table';
    ...
    {
        if (is_object($cols) && method_exists($cols, 'toArray')) {
            $cols = $cols->toArray();
        }
    ...
    public function toString($indent = null)
    {
        ...
        if (count($this->_rows) || $this->_content) {
            ...
            foreach ($this->_rows as $r) {
                ...
                foreach ($this->_columns as $key=>$value) {
                    ...
                    if ($this->_cellContent[$key]) {

                        $this->_currentRow = $r;    // reference current row for _replacePlaceholders callback
                        $xhtml .= preg_replace_callback('/\{([a-zA-Z0-9_]+)\}/', array($this, "_replacePlaceholders"), $this->_cellContent[$key]);
                    }
                    elseif ($r[$key]) {
                        $xhtml .= $r[$key];
                    }
    ...
    {
        return ($this->_currentRow[$matches[1]]) ? $this->_currentRow[$matches[1]] : '';
    }
